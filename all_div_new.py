...
'''def align_predictions(predictions: np.ndarray, label_ids: np.ndarray, valid_ids_array: np.ndarray, id2label: Dict[int, str]) -> Tuple[List[List[str]], List[List[str]]]:
    if isinstance(predictions, (tuple, list)):
        predictions = predictions[0]
    preds = predictions.argmax(-1)
    preds_list, labels_list = [], []
...
def build_compute_metrics(id2label: Dict[int, str]):
    def compute_metrics(p: EvalPrediction) -> Dict[str, float]:
        if p.inputs is not None and isinstance(p.inputs, dict) and 'valid_ids' in p.inputs:
            valid_ids_array = p.inputs['valid_ids'].numpy()
        else:
            valid_ids_array = np.ones_like(p.label_ids)
                                                                                                
        preds_list, gold_list = align_predictions( p.predictions, p.label_ids, valid_ids_array, id2label)
        report = classification_report(gold_list, preds_list, output_dict=True, zero_division=0)
        metrics: Dict[str, float] = {}
        for lbl, stats in report.items():
            if isinstance(stats, dict) and lbl not in {"accuracy","macro avg","weighted avg", "micro avg",}:
                metrics[f"{lbl}_precision"] = stats["precision"]
                metrics[f"{lbl}_recall"] = stats["recall"]
                metrics[f"{lbl}_f1"] = stats["f1-score"]
        if "macro avg" in report:
...
def main() -> None:
    ...
    teacher = teacher.to(t_args.device)
    teacher.eval()
    #teacher.to(device)
    #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #teacher.to("cpu")
    ...
    student = student.to(t_args.device)
    ...
    if trainer.proj is not None:
        trainer.proj.to(device)
    ...
    if t_args.do_eval and dev_ds is not None:
        ...
        #teacher
        logger.info("*** Loading fresh teacher model for evaluation ***")
        eval_teacher = AutoModelForTokenClassification.from_pretrained("./out_base",config=teacher_cfg, cache_dir=m_args.cache_dir,)
        eval_teacher.to(t_args.device)
        eval_teacher.eval()
        dev_ds = NerDataset(data_dir=d_args.data_dir, tokenizer=tokenizer, labels=labels, max_seq_length=d_args.max_seq_length, overwrite_cache=True, mode=Split.dev,)
        teacher_args = deepcopy(t_args)
        teacher_args.remove_unused_columns = True
        teacher_dev_ds = NerDataset(data_dir=d_args.data_dir, tokenizer=tokenizer, labels=labels, max_seq_length=d_args.max_seq_length, overwrite_cache=True, mode=Split.dev,)
